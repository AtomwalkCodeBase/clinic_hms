/**
 * pages/platform-admin/MilestoneTemplatesPage.jsx
 * ----------------------------------------------------
 * Platform admin: manage system-level developmental-milestone schedule
 * templates (MilestoneSchedule rows with owner_tenant_id=None, is_template=
 * True) that every hospital admin sees as "available templates" to clone
 * from (see pages/hospital-admin/MilestoneSchedulePage.jsx).
 *
 * Structural mirror of VaccinationTemplatesPage.jsx — see that file's
 * docstring for the duplication-over-shared-component rationale, which
 * applies identically here.
 *
 * Backend (apps/platform_admin/milestone_template_views.py):
 *   GET    /platform/milestone-templates/        -> [ {id, name, description,
 *                                                        active, rule_count,
 *                                                        tenants_using, ...} ]
 *   POST   /platform/milestone-templates/        -> create {name, description?, active?, rules?}
 *   GET    /platform/milestone-templates/<id>/   -> template + rules[]
 *   PATCH  /platform/milestone-templates/<id>/   -> {name?, description?, active?,
 *                                                      rules?: [{id?, domain, milestone,
 *                                                      scheduled_label, min_age_days,
 *                                                      max_age_days?, mandatory?, sort_order?, _delete?}]}
 *   DELETE /platform/milestone-templates/<id>/   -> refused (400) if any tenant's
 *                                                     active_milestone_schedule_id points at it
 */
...
function TemplateEditor({ template, onSaved, onDeleted, onBack }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { data: res } = await api.patch(API_ENDPOINTS.PLATFORM.MILESTONE_TEMPLATE(template.id), {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const { data: res } = await api.delete(API_ENDPOINTS.PLATFORM.MILESTONE_TEMPLATE(template.id));
...
function NewTemplateForm({ onCreated, onCancel }) {
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      const { data: res } = await api.post(API_ENDPOINTS.PLATFORM.MILESTONE_TEMPLATES, {
  ...
  return (
    <div className="card" style={{ padding: 20, marginBottom: 20 }}>
      <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>New Milestone Template</div>
      <div style={{ display: "grid", gap: 14, marginBottom: 16 }}>
        <div>
          <label style={labelStyle}>Template Name *</label>
          <input style={inputStyle} value={name} onChange={e => setName(e.target.value)}
            placeholder="e.g. Standard Developmental Checklist" autoFocus />
        </div>
        <div>
          <label style={labelStyle}>Description</label>
          <textarea style={{ ...inputStyle, minHeight: 60, resize: "vertical" }} value={description}
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button type="button" className="btn-primary" onClick={handleCreate} disabled={creating}>
          {creating ? "Creating…" : "Create & Add Milestones"}
        </button>
...
export default function MilestoneTemplatesPage() {
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await api.get(API_ENDPOINTS.PLATFORM.MILESTONE_TEMPLATES);
      setTemplates(res.data || []);
    } catch (err) {
      toastApiError(err, "Failed to load milestone templates.");
      setTemplates([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchDetail = useCallback(async (id) => {
    ...
    try {
      const { data: res } = await api.get(API_ENDPOINTS.PLATFORM.MILESTONE_TEMPLATE(id));
          ...
          <>
            ...
            {templates.length === 0 ? (
              <div className="card" style={{ textAlign: "center", padding: 40, color: "var(--color-text-muted)" }}>
                No system milestone templates yet. Create one to give hospitals a starting checklist.
              </div>
            ) : (

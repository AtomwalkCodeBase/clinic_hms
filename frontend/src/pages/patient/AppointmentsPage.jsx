  ...
  return (
    <div className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 22 }}>
      <div style={{
        ...
        padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10,
      }}>
      ...
      <div style={{ padding: "22px 24px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center", marginBottom: 16 }}>
          ...
          {b.token_number != null && (
            <div style={{
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: "var(--color-accent)" }}>Token</span>
        ...
        {b.chief_complaint && (
          <div style={{ fontSize: 13, color: "var(--color-text-secondary)", marginBottom: 16, fontStyle: "italic" }}>
            "{b.chief_complaint}"
          </div>
        )}
          ...
          {[
            ...
            <div key={step.key} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
              <span style={{
                ...
                background: step.done ? "var(--color-success)" : step.active ? "var(--color-accent)" : "var(--color-border)",
...
function AppointmentCard({ b, onOpenPrescriptions, ownAwpid }) {
  ...
  return (
    <div className="card" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {b.chief_complaint && (
        <div style={{ fontSize: 12, color: "var(--color-text-muted)" }}>{b.chief_complaint}</div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 2 }}>
        {b.token_number != null ? (
          <div style={{ fontSize: 12, fontWeight: 700, color: "var(--color-primary)" }}>Token #{b.token_number}</div>
        ) : <span />}
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          {/* Every closed-out appointment (completed, no-show, cancelled, or
              expired without ever being closed) can lead into a follow-up
              booking — deep-linked straight to the same doctor when we know
              which one (doctor_id comes from the underlying Appointment row;
              falls back to the hospital's doctor list if that lookup failed). */}
          {isPast && b.tenant_id && (
            <Link
              to={b.doctor_id
      ...
      <PageShell title="My Appointments">
        ...
        {bookingsLoading ? (
          ...
          <>
            ...
            {/* CTA into the browse flow */}
            <div className="card" style={{
              marginBottom: 22, padding: "20px 24px", overflow: "hidden", position: "relative",
              background: "linear-gradient(135deg, var(--color-primary-light) 0%, var(--color-surface) 100%)",
              display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap",
            }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 17, marginBottom: 4 }}>
                  <Sparkles size={16} style={{ color: "var(--color-accent)" }} /> Need medical help?
                </div>
                ...
                <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                  {stats?.doctors > 0 && (
                    <span style={{ fontSize: 12, fontWeight: 600, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 5 }}>
                      <Users2 size={13} style={{ color: "var(--color-primary)" }} /> {stats.doctors} doctors available
                    </span>
                  )}
                  {stats?.hospitals > 0 && (
                    <span style={{ fontSize: 12, fontWeight: 600, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 5 }}>
                      <Building2 size={13} style={{ color: "var(--color-primary)" }} /> {stats.hospitals} hospitals on the platform
                    </span>
                  )}
                  <span style={{ fontSize: 12, fontWeight: 600, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 5 }}>
                    <CircleCheck size={13} style={{ color: "var(--color-primary)" }} /> Instant confirmation
                  </span>
                </div>
              </div>
              <Link to={ROUTES.PATIENT.HOSPITALS} className="btn-primary" style={{ padding: "10px 24px", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {bookings.length === 0 ? (
              ...
              <>
                {upcoming.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                      <span className="dot-label dot-label--green">Upcoming Appointments ({upcoming.length})</span>
                ...
                {past.length > 0 && (
                  <div>
                    <span className="dot-label dot-label--gold" style={{ marginBottom: 10, display: "inline-block" }}>
